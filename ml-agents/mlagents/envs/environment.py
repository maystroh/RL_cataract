...
class UnityEnvironment(object):
    def __init__(self, file_name=None, worker_id=0,
                 ...
                 visual_obs=False, no_graphics=False):
        """
        ...
        :param use_visual_obs: Informs this class whether the process is using a visual observation
    ...
    def executable_launcher(self, file_name, use_visual_obs, no_graphics):
        cwd = os.getcwd()
        ...
        if launch_string is None:
            ...
            if not use_visual_obs:
                if no_graphics:
                    logger.info("Launch unity -nographics and with vector input")
                    self.proc1 = subprocess.Popen(
                        [launch_string,'-nographics', '-batchmode',
                         '--port', str(self.port)])
                else:
                    logger.info("Launch unity graphics and with vector input")
                    self.proc1 = subprocess.Popen(
    ...
    def _process_pixels(image_bytes, gray_scale):
        ...
        #image.save(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ".png")
