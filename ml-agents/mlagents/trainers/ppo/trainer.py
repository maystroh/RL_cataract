...
class PPOTrainer(Trainer):
            ...
            if stored_info is not None:
                ...
                if not stored_info.local_done[idx]:
                    for i, _ in enumerate(stored_info.visual_observations):
                        ...
                        self.training_buffer[agent_id]['next_visual_obs%d' % i].append(next_info.visual_observations[i][next_idx])

                    if self.policy.use_vec_obs:
                        self.training_buffer[agent_id]['vector_obs'].append(stored_info.vector_observations[idx])
                        self.training_buffer[agent_id]['next_vector_in'].append(next_info.vector_observations[next_idx])
                    ...
                    if self.policy.use_continuous_act:
                        ...
                        self.training_buffer[agent_id]['action_mask'].append(stored_info.action_masks[idx])
        ...
        for l in range(len(info.agents)):
            ...
            if ((info.local_done[l] or len(agent_actions) > self.trainer_parameters['time_horizon'])
                ...
                if info.local_done[l]:
                    self.stats['cumulative_reward'].append(self.cumulative_rewards.get(agent_id, 0))
                    ...
                    self.episode_steps[agent_id] = 0
                    if self.use_curiosity:
                        self.stats['intrinsic_reward'].append(self.intrinsic_rewards.get(agent_id, 0))
                        self.intrinsic_rewards[agent_id] = 0
    ...
    def update_policy(self):
        ...
        self.training_buffer.update_buffer['advantages'].set( (advantages - advantages.mean()) / (advantages.std() + 1e-10))
