...
class TrainerController(object):
    def __init__(self, env_path, run_id, save_freq, curriculum_folder,
        ...
        :param visual_obs_use: is it running using a visual observation?
        ...
        self.visual_obs = visual_obs_use
    ...
    def _save_model(self, sess, saver, steps=0):
        ...
        self.logger.info('Saved Model: ' + last_checkpoint)
        ...
        for brain_name in self.env.external_brain_names:
            if trainer_parameters_dict[brain_name]['trainer'] == 'imitation':
                ...
                self.trainers[brain_name] = PPOTrainer(
                    sess, self.env.brains[brain_name],
                    self.meta_curriculum.brains_to_curriculums[brain_name].min_lesson_length if self.meta_curriculum else 0,
            ...
            try:
                while any([t.get_step <= t.get_max_steps for k, t in self.trainers.items()]) or not self.train_model:
                    ...
                    take_action_vector, take_action_memories, take_action_text, take_action_value, take_action_outputs \
                        = {}, {}, {}, {}, {}
                    for brain_name, trainer in self.trainers.items():
                        (take_action_vector[brain_name], take_action_memories[brain_name], take_action_text[brain_name], \
                         take_action_value[brain_name], take_action_outputs[brain_name]) = trainer.take_action(curr_info)
                ...
                print('Now the saving the model')
                if self.train_model:
